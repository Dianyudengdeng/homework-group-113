Sbox = [ ]

def rotate_left(num, bits):
    num_bits = 32
    bits %= num_bits
    return ((num << bits) & 0xFFFFFFFF) | ((num & 0xFFFFFFFF) >> (num_bits - bits))

def L(x):
    return x ^ rotate_left(x, 2) ^ rotate_left(x, 10) ^ rotate_left(x, 18) ^ rotate_left(x, 24)
def L_1(x):
    return x ^ rotate_left(x, 13) ^ rotate_left(x, 23)

T_table_1=[]
T_table_2=[]
T_table_3=[]
T_table_4=[]
for element in Sbox:
    T_table_1.append(L((element<<24)&0xFFFFFFFF))
    T_table_2.append(L((element<<16)&0xFFFFFFFF))
    T_table_3.append(L((element<<8)&0xFFFFFFFF))
    T_table_4.append(L(element&0xFFFFFFFF))

print(T_table_1)
print(T_table_2)
print(T_table_3)
print(T_table_4)
